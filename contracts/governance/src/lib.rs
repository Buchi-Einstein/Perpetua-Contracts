...
/// Return the configured grace period in seconds (seconds a proposal may remain
/// executable after its `eta`). Falls back to `DEFAULT_GRACE_PERIOD_SECONDS`
/// when the contract has not been initialised (so views never panic pre-init).
...
impl FluxoraGovernance {
    ...
    pub fn init(
        ...
        env.storage()
            .instance()
            .set(&DataKey::GracePeriod, &DEFAULT_GRACE_PERIOD_SECONDS);
        env.storage()
            .instance()
    ...
    /// Configure the grace period in seconds.
    ///
    /// The grace period is the window after a proposal's `eta`
    /// (`quorum_reached_at + GOVERNANCE_TIMELOCK_SECONDS`) during which it may
    /// still be executed. Attempting to execute a queued proposal whose
    /// `eta + grace_period` timestamp has passed returns
    /// [`GovernanceError::ProposalExpired`]. Lowering the grace period lets an
    /// admin retire stale proposals faster (and reclaim storage rent via
    /// [`prune_expired_proposals`](Self::prune_expired_proposals)); raising it
    /// keeps queued proposals executable for longer.
    ///
    /// # Authorization
