...
pub enum GovernanceError {
    ...
    InvalidCalldata = 20,
    /// A reentrant call back into governance was attempted while a proposal
    /// execution was already dispatching its target call.
    ReentrancyGuard = 21,
}
...
impl FluxoraGovernance {
    ...
    /// - `ReentrancyGuard`: A reentrant `execute` was attempted while a proposal
    ///   dispatch was still in flight (see security note below).
    /// - `ArithmeticOverflow`: proposal age or quorum timelock deadline cannot be represented.
    ///
    /// # Security
